...
Spawn the **primary engine specialist** in parallel (from `docs/project/technical-preferences.md` Engine Specialists, or `.claude/docs/technical-preferences.md` as a Claude mirror in legacy repos) to validate the integration approach:
- Is the proposed audio middleware integration idiomatic for the engine? (e.g., Godot's built-in AudioStreamPlayer vs FMOD, Unity's Audio Mixer vs Wwise, Unreal's MetaSounds vs FMOD, Phaser Sound Manager/Web Audio)
