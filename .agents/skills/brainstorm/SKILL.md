...
- **Target platform**: Use `AskUserQuestion` — "What platforms are you targeting for this game?"
  ...
  Note platform implications if relevant (e.g., mobile means Unity is strongly preferred; console means Godot has limitations; browser-first 2D means Phaser 4 is strongly preferred; web means Phaser or Godot can deploy cleanly depending on scope).

- **Engine experience**: Use `AskUserQuestion` — "Do you already have an engine you work in?"
  Options: `Godot` / `Unity` / `Unreal Engine 5` / `Phaser 4` / `No preference — help me decide`
  - If they pick an engine → record it as their preference and move on. Do NOT second-guess it.
  - If "No preference" → tell them: "Run `/setup-engine` after this session — it will walk you through the full decision based on your concept and platform target." For browser-first 2D concepts, consider Phaser 4 as the primary recommendation when the user's goals emphasize web distribution, TypeScript, fast iteration, small scope, or playable prototypes.
